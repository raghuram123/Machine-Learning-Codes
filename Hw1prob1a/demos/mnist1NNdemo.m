%% Classify the MNIST digits using a one nearest neighbour 
%classifier and Euclidean distance
% See mnistKNNdeno for a simple minded implemenation.
% This demo uses various tricks to scale the method up to 
%large
% data sets.
%%
% PMTKslow
...
number = [100, 500, 1000, 2000, 5000, 10000, 20000, 40000, 60000];
size=length(number);   
n=1;
while n<=size
    loadData('mnistAll');
    ...
    for i=1:nbatches
        t = toc;
        if ~isOctave(), waitbar(i/nbatches,wbar,sprintf('%d of %d Classified\nElapsed Time: %.2f seconds',(i-1)*(ntest/nbatches),ntest,t));
        end
    ...
    % Report
    if ~isOctave(), close(wbar);
    end
    errorRate = mean(ypred ~= ytest);
    fprintf('Error Rate: %.2f%%\n',100*errorRate);
    t = toc; 
    fprintf('Total Time: %.2f seconds\n',t);
    plot(number(n),errorRate,'*-');
